...
public class Primes implements Iterable<Integer> {
	...
	private Iterator<Integer> primeIterator = null;

	@Override
	public Iterator<Integer> iterator() {
		if (primeIterator == null) {
			primeIterator = new PrimesIterator();
		}
	...
	private static class PrimesIterator implements Iterator<Integer> {
		...
		public PrimesIterator() {
			// We have to check if we have the list initialized. If not, we
			// insert two obvious ones.
		...
		private static boolean isPrime(int arg) {
			// We assume that we are calling this method only to find next
			// prime.
		...
		private void computeNext() {
			// We already have the only even prime number, so we seek only odd
			// ones;
			int temp;
		...
		public Integer next() {
			// If we already have one computed, there's no point of computing it
			// again.
